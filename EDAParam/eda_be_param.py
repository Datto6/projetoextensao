...
def secao_visao_geral(input:Path,out: Path):
    ...
    SELECT
        ...
        AVG(CASE 
                WHEN pct_subsidio IN ('inf', '-inf') THEN NULL 
                ELSE CAST(pct_subsidio AS DOUBLE) 
            END) AS media_pct_subsidio
    FROM read_csv(
        '{input}/*.csv',
        all_varchar=true,
        header=true
    ...
    query = f"""
        ...
        .join([f"""
            CASE
                WHEN {col} IN ('inf', '-inf', 'Infinity', '-Infinity') THEN NULL
                ELSE CAST({col} AS DOUBLE)
            END AS {col}_num""" 
        for col in colunas
        ])}
    FROM read_csv('{input}/*.csv',all_varchar=true,header=true)
)

        SELECT 'count' AS estatistica,{",".join([f"COUNT({col}_num) AS {col}" for col in colunas])} FROM dados
        UNION ALL
        
        SELECT'mean',{",".join([f"AVG(CAST({col}_num AS DOUBLE))" for col in colunas])} FROM dados
        UNION ALL
        
        SELECT 'std',{",".join([f"STDDEV(CAST({col}_num AS DOUBLE))" for col in colunas])} FROM dados
        UNION ALL

        SELECT'min',{",".join([f"MIN(CAST({col}_num AS DOUBLE))" for col in colunas])} FROM dados
        UNION ALL

        SELECT'25%',{",".join([f"QUANTILE_CONT(CAST({col}_num AS DOUBLE), 0.25)" for col in colunas])} FROM dados
        UNION ALL

        SELECT'50%', {",".join([f"MEDIAN(CAST({col}_num AS DOUBLE))" for col in colunas])} FROM dados
        UNION ALL

        SELECT'75%',{",".join([f"QUANTILE_CONT(CAST({col}_num AS DOUBLE), 0.75)" for col in colunas])} FROM dados
        UNION ALL

        SELECT'max', {",".join([f"MAX(CAST({col}_num AS DOUBLE))" for col in colunas])} FROM dados
        """
...
def secao_temporal(input:Path,out: Path,sep:str,data_ini:str,data_fim:str):
    ...
    txt_faltantes(out=out,data_ini=data_ini, data_fim=data_fim,diario=diario_dict,minimo=MINIMO_ENTRADAS_BE) #cria arquivo txt com dias faltantes
    ...
    with os.scandir(input) as files:
        for file in files:
            for dia in load_data_spec(file.path,cols_in_use,TIPO,sep,chunksize=100_000):
                ...
                if all(c in dia.columns for c in ["linha", "vl_linha","vl_trans","pct_subsidio","num_cartao","dia_semana","num_carro"]): #se dia tem todas essas colunas
                    ...
                    for linha, grp in dia.groupby("linha"): #construcao de resumo por linha
                        ...
                        carros_unicos[linha].update(grp["num_carro"].dropna())
                        ordem_dias = ["Monday","Tuesday","Wednesday","Thursday","Friday","Saturday","Sunday"]
                        nomes_pt   = ["Seg","Ter","Qua","Qui","Sex","Sáb","Dom"]
                        map_dias   = dict(zip(ordem_dias, nomes_pt)) #apenas p abreviar dias de semana
...
def EDA_BE(input,output,sep,data_ini,data_fim):
